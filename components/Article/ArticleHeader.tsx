...
export default function ArticleHeader({ data }: Props) {
    ...
    return (
        <header className="w-full px-[5%] py-8 md:py-12 lg:py-16 font-sans">
        
            <div className="max-w-5xl mx-auto">
                {/* Category & reading time */}
                <div className="flex items-center gap-4 mb-4 sm:mb-6">
                    {isFilled.contentRelationship(category) && isFilled.keyText(category.data.category_name) && (
                    <span className="bg-[var(--color-surface)] text-[var(--color-text-primary)] text-[12px] sm:text-sm font-medium px-4 py-1.5 rounded-lg">
                        {category.data.category_name}
                    </span>
                    )}
                    {isFilled.number(reading_time_estimation) && (
                    <span className="text-[var(--color-text-primary)] text-[12px] sm:text-sm font-medium">
                ...
                {isFilled.image(image) && (
                    <div className="w-full aspect-[16/9] md:aspect-[21/9] overflow-hidden rounded-xl shadow-(--shadow-primary) bg-gray-200 mb-10">
                    <PrismicNextImage 
                        field={image} 
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                    ...
                    {isFilled.contentRelationship(author) && (
                        ...
                        {isFilled.image(author.data.author_picture) && (
                            <div className="w-10 h-10 overflow-hidden rounded-full shadow-(--shadow-primary) bg-gray-200">
                            <PrismicNextImage field={author.data.author_picture} className="w-full h-full object-cover" />
                            </div>
                        )}
                        <div>
                            <p className="text-[12px] sm:text-sm text-[var(--color-text-secondary)] mb-0.5">Written by</p>
                    ...
                    {isFilled.date(publication_date) && (
                        <div>
                            <p className="text-[12px] sm:text-sm text-[var(--color-text-secondary)] mb-0.5">Published on</p>
